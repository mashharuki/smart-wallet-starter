...
export class Multicall {
    private multicallContractAddress =
    ...
    constructor(options: InitOptions) {
        this.contract = new Contract(
            ...
            abiMulticall,
            getChainProvider(options.chainId),
        );
    }
    ...
    private async _getTokenBalances(
        user: string,
        addresses: Array<string>,
    ): Promise<MulticallGetBalancesResponse | null> {
        const returnData: MulticallGetBalancesResponse = [];
        ...
        const baseCallData = erc20TokenInterface.encodeFunctionData(
            'balanceOf',
            [user],
        );
        ...
        const params: Array<MulticallParams> = addresses.map((address) => {
            ...
            if (address.toLowerCase() === this.getETHAddress()) {
                ...
                callData = BALANCEOF_UINT256_SELECTOR.concat(
                    baseCallData.slice(10),
                );
            }
